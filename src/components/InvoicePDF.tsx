...
const InvoicePDF = ({ order }: InvoicePDFProps) => {
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      doc.text(`${Number(order.total_amount).toLocaleString()} ${isRTL ? "ر.س" : "SAR"}`, 165, 180, { align: "right" });
      ...
      doc.setTextColor(30, 64, 175);
      doc.text(`${Number(order.total_amount).toLocaleString()} ${isRTL ? "ر.س" : "SAR"}`, 165, 205, { align: "right" });
